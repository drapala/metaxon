...
- Appends incoming messages to FIFO queue
- Triggers LLM inference on new messages
- Writes both incoming and generated messages to recall storage
- **Memory pressure warnings**: at 70% context capacity, inserts system warning
- **Queue eviction**: at 100% capacity, flushes 50% of messages, generates recursive summary
- Evicted messages stored in recall storage (never lost, retrievable via function calls)

### Control Flow via Events

Event-driven loop (not turn-based):
- User messages, system messages, user interactions, and timed events trigger processing
- Agent can chain multiple memory operations before responding via `request_heartbeat=true`
- Function call results feed back into next processing step
- Enables multi-step retrieval within a single turn

## Key Contributions

1. **Unbounded context illusion**: documents exceeding context window analyzed via pagination + memory management
2. **Multi-session continuity**: agents maintain coherent personality and knowledge across conversations
3. **Self-directed memory**: agent decides what to store, retrieve, and forget — not a fixed pipeline

## Evolution into Letta

MemGPT evolved into the **Letta** framework, which formalized the 4-block memory architecture:
- **Core Memory** (always in context): identity, system instructions, critical facts
- **Message Buffer** (rolling window): recent conversation
- **Archival Memory** (unbounded): long-term vector/text storage
- **Recall Memory** (on-demand): rehydrates archived knowledge via agent-initiated queries

## Relevance to Knowledge Bases

MemGPT's virtual context management is the foundational concept behind bandwidth-aware retrieval. The principle "never load everything into context — page in what you need" directly maps to the 3-layer /ask pattern (index → articles → raw). MemGPT proves this works for agents; a KB applies it to knowledge retrieval.

The self-directed memory operations (agent decides what to archive/retrieve) parallel the autonomy in /ingest (LLM decides concepts) and /review (LLM decides what to consolidate/prune).

The queue manager's memory pressure warnings and eviction policy mirror Claude Code's auto-compaction triggers (context_window - 13,000 tokens).
...
working_context.append(“Birthday
is February
7”) 7”)
working_context.append(“Birthday
is February
working_context.append(“Boyfriend
namednamed
James”)
working_context.append(“Boyfriend
James”)
...
went to
ﬂags! is
size of ∼50 tokens (∼250 characters). ‘Open’
thesixmodel
...
Llama (1) [01/14/2024] “i love six ✓
2ktimes”,
20
ﬂags been like 100
Llama 2 [10/12/2023] “james and✓I actually ﬁrst met
4kat six ﬂags” 60
...
Küttler, Mike Lewis, Wen-tau Yih, Tim Rocktäschel,
...
Timo Schick, Jane Dwivedi-Yu, Roberto Dessı̀, Roberta
